...
module.exports = {
     ...
     * @apiParam {String} role role (superadmin, admin, decideur)
    ...
    create(req, res) {
        ...
        const NewBoUser = new BackOfficeUser({
    ...
    auth(req, res) {
        ...
        BackOfficeUser.findOne({
            ...
            bcrypt.compare(password, user.password).then(function (result) {
                if (result) {
                    const token = jwt.sign({ boUserID: user._id });
                    res.status(200).send({
                        success: true,
                        userId: user._id,
    ...
    updatePassword(req, res) {
        ...
        BackOfficeUser.findById(req.boUserID).then((user) => {

            bcrypt.compare(oldPassword, user.password).then(function (result) {
                if (result) {
                    const filter = { password: bcrypt.hashSync(newPassword, saltRounds) };
                    const update = { active: 'active' };
                    BackOfficeUser.findOneAndUpdate(filter, update)
                        .then((user) => {
                            res.status(201).send({
                                success: true,
                                message: 'Successfully updated.'
     ...
     */
    getAll(req, res) {
        BackOfficeUser.find().populate('backOfficeUser')
            .then((users) => {
                res.status(200).send({
                    users,
                });
    ...
    get(req, res) {
        ...
        BackOfficeUser.findById(id).populate('backOfficeUser')
            .then((user) => {
    ...
    changeStatut(req, res) {
        ...
        BackOfficeUser.findById(id).then((user) => {
            const filter = { _id: id };
            const update = { actif: !user.actif };
            BackOfficeUser.findOneAndUpdate(filter, update)
                .then((n) => {
    ...
    update(req, res) {
        ...
        const filter = { _id: id };
        BackOfficeUser.findOneAndUpdate(filter, data)
            .then((user) => {
                res.status(201).send({
                    success: true,
                    message: 'Successfully updated.'
